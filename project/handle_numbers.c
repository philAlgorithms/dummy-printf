...
/**
 * handle_di - Converts an argument to a signed int and
...
/**
 * handle_b - Converts an unsigned int argument to binary
		...
		unsigned char flags, int wid, int prec, unsigned char len)
{
	...
	return (handle_ubase(output, num, "01", flags, wid, prec));
}

/**
 * handle_o - Converts an unsigned int to octal and
 *             stores it to a buffer contained in a struct.
		...
		unsigned char flags, int wid, int prec, unsigned char len)
{
	...
	if (!(num == 0 && prec == 0))
		ret += handle_ubase(output, num, "01234567",
				flags, wid, prec);
...
/**
 * handle_u - Converts an unsigned int argument to decimal and
 *               stores it to a buffer contained in a struct.
 * @args: A va_list pointing to the argument to be converted.
 * @flags: Flag modifiers.
 * @wid: A width modifier.
		...
		unsigned char flags, int wid, int prec, unsigned char len)
{
	...
	if (!(num == 0 && prec == 0))
		ret += handle_ubase(output, num, "0123456789",
				flags, wid, prec);
