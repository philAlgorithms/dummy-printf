...
unsigned int handle_s(va_list args, buffer_t *output,
		unsigned char flags, int wid, int prec, unsigned char len);
unsigned int handle_S(va_list args, buffer_t *output,
		unsigned char flags, int wid, int prec, unsigned char len);
unsigned int handle_r(va_list args, buffer_t *output,
		unsigned char flags, int wid, int prec, unsigned char len);
unsigned int handle_R(va_list args, buffer_t *output,
		unsigned char flags, int wid, int prec, unsigned char len);

/**
 * handle_s - Converts an argument to a string and
		...
		unsigned char flags, int wid, int prec, unsigned char len)
{
	...
	for (index = 0; *(str + index) != '\0' && index < prec; index++)
	{
		if (*(str + index) < 32 || *(str + index) >= 127)
		{
			...
			ret += handle_ubase(output, *(str + index),
...
/**
 * handle_r - Reverses a string and stores it
...
/**
 * handle_R - Converts a string to ROT13 and stores
